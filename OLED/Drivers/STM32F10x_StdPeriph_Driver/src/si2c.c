...
// @简介：对软件I2C进行初始化
//
__attribute__((weak)) void My_SI2C_Init(SI2C_TypeDef *SI2C)
{
	// #1. 使能SCL引脚的时钟
...
//
__attribute__((weak)) int My_SI2C_SendBytes(SI2C_TypeDef *SI2C, uint8_t Addr, const uint8_t *pData, uint16_t Size)
{
	sda_w(1);
	scl_w(1);
...
// @返回值：0 - 发送成功， -1 - 寻址失败
//
__attribute__((weak)) int My_SI2C_ReceiveBytes(SI2C_TypeDef *SI2C, uint8_t Addr, uint8_t *pBuffer, uint16_t Size)
{
	sda_w(1);
	scl_w(1);
