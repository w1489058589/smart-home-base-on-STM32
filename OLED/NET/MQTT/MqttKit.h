...
typedef struct Buffer
{
	...
	uint8	_memFlag;	//内存使用的方案：0-未分配1一使用的动态分配2-使用的固定内存
...
enum MqttPacketType
{
    ...
    MQTT_PKT_PUBREC,      /**发布数据已接收数据包，QoS2时，回复MQTT_PKT_PUBLIS */
    MQTT_PKT_PUBREL,      /**发布数据释放数据包，QoS2时，回复MQTT_PKT_PUBREC*/
    MQTT_PKT_PUBCOMP,     /**发布完成数据包，QoS2时，回复MQTT_PKT_PUBRE */
    MQTT_PKT_SUBSCRIBE,   /**订阅数据包 */
    MQTT_PKT_SUBACK,      /**订阅确认数据包 */
    MQTT_PKT_UNSUBSCRIBE, /**取消订阅数据包 */
    MQTT_PKT_UNSUBACK,    /**取消订阅确认数据包 */
    MQTT_PKT_PINGREQ,     /**ping数据包 */
    MQTT_PKT_PINGRESP,    /**ping响应数据包 */
